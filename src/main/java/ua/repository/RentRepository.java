...
//    -------------------------query for LendController --------------------------------
    ...
    @Query("select new ua.model.view.DriverView(d.id, d.name, d.surname, d.phone, d.countOfTrips, p.name, d.dateOfBirth, d.experienceBegan)" +
...
//    -----------------------query for BorrowController---------------------
    ...
    @Query("select new ua.model.view.OrderView(o.id,  o.pricePerDay, o.periodOfRentFrom, o.periodOfRentTo, r.name, o.chauffeur, o.status)" +
            "from InfoAboutRent o join o.region r join o.car c join c.driver d join d.user u where u.email <> ?1 and o.status= ?2 order by o desc")
    List<OrderView> findAllFreeCars(String email, Status status);

    @Query("select new ua.model.view.OrderView(o.id, ds.id, o.pricePerDay, o.periodOfRentFrom, o.periodOfRentTo, r.name, o.chauffeur, o.status)" +
            "from InfoAboutRent o join o.drivers ds join o.region r where ds.id = (select d.id from Driver d join d.user u where u.email=?1) and (o.status = ?2 or o.status=?3)  order by o desc")
    List<OrderView> findOrdersByDriverId(String email, Status active, Status completed);
}
